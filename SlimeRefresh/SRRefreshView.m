...
{
    CGRect frame = CGRectMake(0, 100, 320, height);
...
//        
        ...
        _webView = [[UIWebView alloc] initWithFrame:CGRectMake(self.frame.size.width * 0.5 - 10, 74, 20, 20)];
        _webView.backgroundColor = [UIColor redColor];
        _webView.center = _slime.startPoint;
...
- (void)pullApart:(SRRefreshView*)refreshView
{
    ...
    if ([_delegate respondsToSelector:@selector(slimeRefreshStartRefresh:)]) {
        [self.delegate slimeRefreshStartRefresh:self];
...
- (void)restore
{
    ...
    NSData *data = [NSData dataWithContentsOfFile:[[NSBundle mainBundle]pathForResource:@"smallrun" ofType:@"gif"]];
