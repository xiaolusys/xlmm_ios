...
@interface JMSystemPermissionsManager () <CLLocationManagerDelegate,UIAlertViewDelegate>

@property(nonatomic,strong) CLLocationManager *locationManager;

@end

@implementation JMSystemPermissionsManager

+ (instancetype)sharedManager {
    static dispatch_once_t onceToken = 0;
    dispatch_once(&onceToken, ^{
        systemPermissionsManager = [[JMSystemPermissionsManager alloc] init];
    });
...
- (void)executeAlterTips:(NSString *)alterTips isSupport:(BOOL)isSupport{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (isSupport) {
            ...
            [JMBlockAlertView alertWithTitle:alterContent
                                   message:@""
