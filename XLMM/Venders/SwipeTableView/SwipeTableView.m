...
- (void)layoutSubviews {
    ...
    self.swipeHeaderBar.cs_top = _swipeHeaderView.cs_bottom;
    if (_swipeHeaderBarScrollDisabled) {
        _swipeHeaderBar.cs_top = _swipeHeaderTopInset;
    }
}
...
- (void)setSwipeHeaderView:(UIView *)swipeHeaderView {
    if (_swipeHeaderView != swipeHeaderView) {
        ...
        _swipeHeaderView.cs_y += _swipeHeaderTopInset;
...
- (void)setSwipeHeaderBar:(UIView *)swipeHeaderBar {
    if (_swipeHeaderBar != swipeHeaderBar) {
        ...
        _swipeHeaderBar.cs_y += _swipeHeaderTopInset;
...
- (void)setSwipeHeaderTopInset:(CGFloat)swipeHeaderTopInset {
    if (_swipeHeaderView) {
        _swipeHeaderView.cs_y += (swipeHeaderTopInset - _swipeHeaderTopInset);
    }
    if (_swipeHeaderBar) {
        _swipeHeaderBar.cs_y += (swipeHeaderTopInset - _swipeHeaderTopInset);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector(swipeTableView:viewForItemAtIndex:reusingView:)]) {
        ...
        if (!setHeaderHeight) {
            ...
            CGFloat deltHeaderHeight = headerHeight - headerView.cs_h;
            headerView.cs_h += deltHeaderHeight;
            if (deltHeaderHeight != 0) {
                newSubView.headerView = headerView;
            }
        }
        contentInset.top = headerView.cs_h + _swipeHeaderTopInset;
        newSubView.scrollIndicatorInsets = contentInset;
...
#if !defined(ST_PULLTOREFRESH_HEADER_HEIGHT)
    CGFloat minRequireHeight = itemView.cs_h - (_swipeHeaderTopInset + _barInset);
#else
    CGFloat minRequireHeight = itemView.cs_h - _swipeHeaderTopInset + _headerInset;
#endif
    ...
    if (context == SwipeTableViewItemContentOffsetContext) {
        ...
        if (!_swipeHeaderBarScrollDisabled) {
            ...
            if (newOffsetY < - topMarginOffset) {
                if (_swipeHeaderBar) {
                    _swipeHeaderBar.cs_bottom  = - newOffsetY;
                    _swipeHeaderView.cs_bottom = _swipeHeaderBar.cs_top;
                }else {
                    _swipeHeaderView.cs_bottom = - newOffsetY;
                }
            }else {
                _swipeHeaderBar.cs_bottom  = topMarginOffset;
                ...
                _swipeHeaderView.cs_bottom = fmax(- (newOffsetY + _barInset), 0);
            }
#else
            ...
            if (newOffsetY < topMarginOffset) {
                if (_swipeHeaderView) {
                    _swipeHeaderView.cs_top = - newOffsetY;
                    _swipeHeaderBar.cs_top  = _swipeHeaderView.cs_bottom;
                }else {
                    _swipeHeaderBar.cs_top  = - newOffsetY;
                }
            }else {
                _swipeHeaderBar.cs_top  = _swipeHeaderTopInset;
                // 'fmax' is used to fix the bug below iOS8.3 : the position of the bar will not correct when the swipeHeaderView is outside of the screen.
                _swipeHeaderView.cs_top = fmax(- newOffsetY, - _headerInset);
            }
#endif
        }
...
#endif
    _swipeHeaderBar.cs_top = fmax(_swipeHeaderTopInset, headerView.cs_bottom);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    NSInteger currentItemIndex = offsetX/scrollView.cs_w + 0.5;
