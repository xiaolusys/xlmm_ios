...
#define kBgImgX             75 * ratio
#define kBgImgY             (64+60) * ratio
#define kBgImgWidth         225 * ratio
...
#define kLampWidth          65 * ratio
#define kLampHeight         105 * ratio
#define kBgAlpha            0.6
...
- (void)viewDidLoad {
    ...
    [self createNavigationBarWithTitle:@"二维码/条形码" selecotr:@selector(backClick:)];
...
//    self.tip = tipLabel;
    [tipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(weakSelf.bgImg.mas_centerX);
    }];
    
    UIView *groupView = [UIView new];
    [self.view addSubview:groupView];
    [groupView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(tipLabel.mas_bottom).offset(20);
        make.centerX.equalTo(weakSelf.bgImg.mas_centerX);
        make.width.mas_equalTo(@(kBgImgWidth));
        make.height.mas_equalTo(@(kLampHeight));
    }];
    
    NSArray *buttonArrayNomal = @[@"qrcode_scan_btn_photo_on",@"qrcode_scan_btn_flash_on"]; //,@"qrcode_scan_btn_myqrcode_on"
    NSArray *buttonArraySelected = @[@"qrcode_scan_btn_photo_on",@"qrcode_scan_btn_flash_off"]; //,@"qrcode_scan_btn_myqrcode_off"
    NSInteger numCode = buttonArrayNomal.count;
    CGFloat buttonWidth = kBgImgWidth / numCode;
    for (int i = 0 ; i < numCode; i++) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        [groupView addSubview:button];
        button.alpha = kBgAlpha;
        button.selected = NO;
        button.tag = 100 + i;
        [button setImage:[UIImage imageNamed:buttonArrayNomal[i]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:buttonArraySelected[i]] forState:UIControlStateSelected];
        [button addTarget:self action:@selector(mamaButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        
        
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(groupView);
            make.left.equalTo(groupView).offset((i % numCode) * buttonWidth);
...
- (void)touchLamp:(id)sender {
    ...
    if (btn.selected == YES) {
        [btn setImage:[UIImage imageNamed:@"qrcode_scan_btn_flash_off"] forState:UIControlStateNormal];
//        btn.backgroundColor = [UIColor whiteColor];
    }else{
        [btn setImage:[UIImage imageNamed:@"qrcode_scan_btn_flash_on"] forState:UIControlStateNormal];
 ...
 *  - (BOOL)isCanUsePhotos {
 ...
 if ((status = ALAuthorizationStatusRestricted | ALAuthorizationStatusDenied)) {
 return NO;
 }
 }else {
 ...
 UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"无权访问相册" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
 [alert show];
 */
