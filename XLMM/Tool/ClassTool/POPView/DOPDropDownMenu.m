...
#define kCellBgColor [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1]
#define kCellSelectedBgColor [UIColor colorWithRed:246/255.0 green:246/255.0 blue:246/255.0 alpha:1]
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    if (self.segmentType == segmentTypeImage) { // 只有图片的情况下,只考虑只有一张图片的情况
        UIImageView *imageV = [UIImageView new];
        [self addSubview:imageV];
        imageV.frame = CGRectMake(self.frame.size.width / 2 - 12, self.frame.size.height / 2 - 12, 24, 24);
        ...
        CGFloat separatorLineInterval = SCREENWIDTH / _numOfMenu;
...
- (instancetype)initWithOrigin:(CGRect)rect andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        //lefttableView init
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREENWIDTH/2, 0) style:UITableViewStylePlain];
        ...
        //righttableView init
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(0 + SCREENWIDTH/2, self.frame.origin.y + self.frame.size.height, SCREENWIDTH/2, 0) style:UITableViewStylePlain];
        ...
        _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREENWIDTH, kButtomImageViewHeight)];
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(0, origin.y + self.frame.size.height, SCREENWIDTH, SCREENHEIGHT)];
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(0, 0, SCREENWIDTH/self.numOfMenu, self.frame.size.height-1);
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    //calculate index
    NSInteger tapIndex = touchPoint.x / (SCREENWIDTH / _numOfMenu);
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        if (haveItems) {
            _leftTableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREENWIDTH/2, 0);
            _rightTableView.frame = CGRectMake(0 + SCREENWIDTH/2, self.frame.origin.y + self.frame.size.height, SCREENWIDTH/2, 0);
            [self.superview addSubview:_leftTableView];
            [self.superview addSubview:_rightTableView];
        } else {
            _leftTableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREENWIDTH, 0);
            _rightTableView.frame = CGRectMake(0 + SCREENWIDTH/2, self.frame.origin.y + self.frame.size.height, SCREENWIDTH/2, 0);
            [self.superview addSubview:_leftTableView];
            
        }
        _buttomImageView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREENWIDTH, kButtomImageViewHeight);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                _leftTableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREENWIDTH/2, tableViewHeight);
                
                _rightTableView.frame = CGRectMake(0 + SCREENWIDTH/2, self.frame.origin.y + self.frame.size.height, SCREENWIDTH/2, tableViewHeight);
            } else {
                _leftTableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREENWIDTH, tableViewHeight);
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        bg.backgroundColor = kCellSelectedBgColor;
    ...
    if (tableView == _leftTableView) {
        ...
        if (_dataSourceFlags.titleForRowAtIndexPath) {
            ...
            if (_dataSourceFlags.imageNameForRowAtIndexPath) {
                imageName = [_dataSource menu:self imageNameForRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        ...
        if (self.segmentType == segmentTypeImage) {
            if (indexPath.row == currentSelectedMenudRow)
            {
                cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"categoryPauxu"]];
                if (cell.imageView) {
                    cell.imageView.image = [UIImage imageNamed:self.selectedImageArray[currentSelectedMenudRow]];
                }
                [tableView selectRowAtIndexPath:indexPath animated:YES scrollPosition:UITableViewScrollPositionNone];
            }else {
                if (cell.imageView) {
                    cell.imageView.image = [UIImage imageNamed:self.selectedImageArray[indexPath.row]];
                }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_didSelectedIndex == indexPath.row) {
        return ;
    }
